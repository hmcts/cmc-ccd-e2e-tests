...
Feature("Judge Draw Direction Order (JDDO - claim amount > 500)").retry(
...
async function runFeatureTestSteps(I, createCitizenCaseJson) {
  ...
  await caseHelper.updateApiEvent(
    caseEventId.ASSIGN_FOR_JUDGE_DIRECTIONS,
    updatedCaseJson,
    caseId
  );
  logger.info({
    message: "Case is updated to READY FOR DIRECTIONS - JUDGE: ",
    caseId,
  });
