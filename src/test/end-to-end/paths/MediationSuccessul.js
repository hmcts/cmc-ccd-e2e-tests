...
Scenario(
  'Full Defence → Dispute All → Accept mediation by Defendant → Decide to proceed is Yes (claimant)→ Accept mediation by claimant',
  async ({ I }) => {
...
async function runFeatureTestSteps(I, createCitizenCaseJson) {
  ...
  await caseHelper.updateCaseworkerEvent(
    I,
    caseEventName.REFERRED_MEDIATION,
    caseId,
  );
  logger.info({
    message: 'Case status changed to REFFERED_MEDIATION for ',
    caseId,
  });

  await caseHelper.updateMediationSuccessful(I);
  logger.info({ message: 'Case is updated to MEDIATION_SUCCESSFUL: ', caseId });
