...
module.exports = async function (
  formType = 'OCON9x (Paper response (All))',
  docSubmissionFieldEnabled = false,
) {
  const I = this;
  await I.waitInUrl(
    'ReviewedPaperResponse/ReviewedPaperResponse1',
  ...
  await I.attachFile(
    paperRespReviewedConfig.page1_staffUploadDocField,
    'fixtures/data/test.pdf',
  );
  await I.waitForElement(
    '#scannedDocuments_0_type',
    testConfig.TestTimeToWaitForText,
  );
  await I.selectOption('#scannedDocuments_0_type', 'Form');
  await I.waitForElement(
    '#scannedDocuments_0_formSubtype',
    testConfig.TestTimeToWaitForText,
  );
