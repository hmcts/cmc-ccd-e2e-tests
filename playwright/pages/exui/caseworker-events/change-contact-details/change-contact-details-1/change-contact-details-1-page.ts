...
import { heading, radioButtons, subHeadings, claimantInputs, defendantInputs, links } from './change-contact-details-1-content';

@AllMethodsStep()
export default class ChangeContactDetails1Page extends ExuiEvent(BasePage) {
  async verifyContent(caseData: CCDCaseData) {
    await super.runVerifications([super.expectHeading(heading), super.verifyCaseTitle(caseData), super.expectLabel(radioButtons.claimant.label, { exact: true }), super.expectLabel(radioButtons.defendant.label, { exact: true })]);
  }
  ...
  async verifyClaimantContent(claimantDetails: ApplicantValue) {
    await super.runVerifications([
      ...
      super.expectInputValue(claimantInputs.email.selector, claimantDetails.partyDetail.emailAddress),
      super.expectInputValue(claimantInputs.addressLine1.selector, claimantDetails.partyDetail.primaryAddress.AddressLine1),
      super.expectInputValue(claimantInputs.addressLine2.selector, claimantDetails.partyDetail.primaryAddress.AddressLine2),
      super.expectInputValue(claimantInputs.postcode.selector, claimantDetails.partyDetail.primaryAddress.PostCode),
    ]);
  }
  ...
  async verifyDefendantContent(defendantDetails: RespondentValue) {
    await super.runVerifications([
      ...
      // super.expectTextToBeVisible(claimantDetails.partyName),
      super.expectInputValue(defendantInputs.email.selector, defendantDetails.claimantProvidedDetail.emailAddress),
      super.expectInputValue(defendantInputs.addressLine1.selector, defendantDetails.claimantProvidedDetail.primaryAddress.AddressLine1),
      super.expectInputValue(defendantInputs.addressLine2.selector, defendantDetails.claimantProvidedDetail.primaryAddress.AddressLine2),
      super.expectInputValue(defendantInputs.postcode.selector, defendantDetails.claimantProvidedDetail.primaryAddress.PostCode),
    ]);
  }
