...
export default abstract class BasePage {
  ...
  protected async clickLink(
    ...
    await this.page
      .getByRole('link', { name, exact: options.exact })
      .nth(options.index ?? 0)
  ...
  protected async expectHeading(
    text: string | number,
    options: { message?: string; exact?: boolean; timeout?: number } = {},
  ) {
    await pageExpect(
  ...
  protected async expectRadioLabel(
    ...
    await pageExpect(
      this.page.locator(`label[for="${inputId}"]`).getByText(label, { exact: options.exact }),
      {
        message: options.message,
      },
    ).toBeVisible({ timeout: options.timeout });
  ...
  protected async expectTableValueByRowName(
    ...
    let locator = this.page
      ...
      .getByRole('cell', { name: value.toString(), exact: options.exact });
    locator = this.getNewLocator(locator, options.containerSelector, options.index, options.first);
